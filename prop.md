...
We’ll use sockets to connect 8 players, then use process control (fork/wait) to run matches while the main server keeps the bracket moving. We’ll use dynamic memory to store player info, boards, and bracket state, and use signals (like SIGCHLD) to clean up finished match processes and handle disconnects cleanly.
